...
find "$CUR_DIR" -type f -exec file {} \; \
	...
	| tr ':' ' ' | awk '{print $1}' \
	| while read -r file; do $X86_STRIP -s "$file"; done && rm "$CUR_DIR/strip"
...
find "$CUR_DIR" -type f -exec file {} \; \
	...
	| tr ':' ' ' | awk '{print $1}' \
	| while read -r file; do $ARM64_STRIP -s "$file"; done && rm "$CUR_DIR/aarch64-linux-gnu-strip"
...
find "$CUR_DIR" -type f -exec file {} \; \
	...
	| tr ':' ' ' | awk '{print $1}' \
	| while read -r file; do $ARM32_STRIP -s "$file"; done && rm "$CUR_DIR/arm-linux-gnueabi-strip"
